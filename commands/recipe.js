    ...
    async execute(client, interaction, MessageEmbed, Formatters, db) {
        ...
        axios.request(options).then(async (response) => {
            ...
            data.forEach(async (recipe) => {
                ...
                if (recipe["total_time_tier"] === null || recipe["total_time_tier"] === undefined) {
                    // Do Nothing
                }
                else {
                    time = `takes ${recipe["total_time_tier"].display_tier} to make`.toLowerCase();
                }

                desc = desc + `[${recipe.name}](https://tasty.co/recipe/${recipe.slug}) (${servings} servings - ${time || "Time tier is unknown"})\n`;
            });
