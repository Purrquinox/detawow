...
module.exports = {
	...
	async execute(client, interaction, MessageEmbed, Formatters, db) {
		...
		let results = await ev.eval("bash", command).then(async (results) => {
			embed = new MessageEmbed()
				.setTitle("Bash Results")
				.setColor("RANDOM")
				.addField("Input:", Formatters.codeBlock("bash", command), inline)
				.addField("Output:", Formatters.codeBlock("bash", limit(results.output || results.message)), inline)
			...
			embed = new MessageEmbed()
				.setTitle("Bash Results")
				.setColor("#FF0000")
				.addField("Input:", Formatters.codeBlock("bash", limit(command)), inline)
				.addField("Output:", Formatters.codeBlock("bash", limit(error)), inline)
				.setFooter({
                    iconURL: interaction.user.displayAvatarURL(),
                    text: `Executed by ${interaction.user.username}, in about ${Math.floor(Date.now() - interaction.createdAt)} milliseconds`
                });
